...
window.addEventListener('load', async () => {
    // --- IMPORTANT: ADD YOUR FINAL CONTRACT DETAILS HERE ---
...
const rpcUrl = "https://sepolia.infura.io/v3/5cb8eb30eaae446c81ab26a22e968dda";
    ...
    // --- Renders blocks in the green card style ---
    const renderBlock = (block) => {
        ...
        if (myTxs.length > 0) {
            ...
            myTxs.forEach(event => {
                ...
                } else if (event.event === 'InstitutionRegistered') {
                     txData += `⭐ REGISTERED:\n  Name: ${args.name}\n  Wallet: ${args.wallet.substring(0,14)}...\n`;
                }
            });
        } else {
    ...
    const fetchData = async () => {
        ...
        try {
            // --- UPDATED: Fetches all event types ---
            const issuedFilter = contract.filters.CertificateIssued();
            const revokedFilter = contract.filters.CertificateRevoked();
            const registeredFilter = contract.filters.InstitutionRegistered();

            const [issuedEvents, revokedEvents, registeredEvents] = await Promise.all([
            ...
            const allEvents = [...issuedEvents, ...revokedEvents, ...registeredEvents];
